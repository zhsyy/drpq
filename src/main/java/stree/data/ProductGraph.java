...
//    private final Logger LOG = LoggerFactory.getLogger(ProductGraph.class);
    ...
    // The purpose of increasing capacity is to avoid wasting space. When the map size overflows, it will automatically expand.
    ...
    public void addEdge(V source, V target, L label, long timestamp) {
        ...
        for(Map.Entry<Integer, Integer> transition : transitions.entrySet()) {
            ...
            edgeCount++;
            degreeCount += sourceNode.getForwardEdges().size();
        }
        if (degreeCount > 500)
            RunningSnapShot.hotVertexCount++;
    }
    ...
    // Both the outgoing and incoming edges are saved, one for forward and the other for backward
